...
def run_audio_metamer_generation(
    ...
    model, ds, metamer_layers = build_network.main(
    ...
    INPUTAUDIOFUNC = generate_import_audio_functions(
        INPUTAUDIOFUNCNAME, rms_normalize=0.1
    )
    ...
    assert audio_dict["SR"] == ds.SR, (
        "Metamer input sound SR is %d while dataset SR is %d"
        % (audio_dict["SR"], ds.SR)
    )
    ...
    label_idx = list(label_keys)[
        list(label_values).index(audio_dict["correct_response"])
    ]
    ...
    orig_cat_prediction = [
        ds.label_mapping[np.argmax(p.ravel())] for p in orig_predictions
    ]

    print("Orig Audio Category Prediction: %s" % (orig_cat_prediction))
    ...
    for layer_to_invert in metamer_layers:
        ...
        print(
            "Step %d | Layer %s | Loss %f" % (0, layer_to_invert, this_loss), flush=True
        )
        ...
        # Get the predicted category label
        synth_cat_prediction = [
            ds.label_mapping[np.argmax(p.ravel())] for p in synth_predictions
        ]

        print(
            "Layer %s, Synth Audio Category Prediction: %s"
    ...
    for layer_idx, layer_to_invert in enumerate(metamer_layers):
        ...
        for i in range(BATCH_SIZE):
            ...
            synth_cat_prediction = ds.label_mapping[
                np.argmax(synth_predictions.ravel())
            ]
            print(
                "Layer %s, Image %d, Orig Word Category Prediction: %s"
                % (layer_to_invert, i, orig_cat_prediction)
            )
            print(
                "Layer %s, Image %d, Synth Word Category Prediction: %s"
            ...
            plt.imshow(
                np.squeeze(np.array(all_outputs["input_after_preproc"][i].cpu())),
                origin="lower",
            )
            plt.colorbar()
            plt.title(
                'Layer %s, Word %d, Predicted Orig Label "%s" \n Orig Coch'
                % (layer_to_invert, i, ds.label_mapping[int(orig_label)])
            )
            ...
            plt.imshow(
                np.squeeze(np.array(all_outputs_out["input_after_preproc"][i].cpu())),
                origin="lower",
            )
            plt.colorbar()
            plt.title(
                'Layer %s, Word %d, Predicted Synth Label "%s" \n Synth Coch'
                % (layer_to_invert, i, ds.label_mapping[int(synth_label)])
            )
            plt.ylabel("Cochleagram Frequency Bins")
            plt.xlabel("Cochleagram Time Bins")
            ...
            try:
                print(
                    'Layer %s, Word %d, Label "%s", Prediction Orig "%s", Prediction Synth "%s"'
                    % (
                        layer_to_invert,
                        i,
            ...
            if layer_idx == 0:
                wavfile.write(
                    base_filepath + "/orig.wav", 20000, im[i].cpu().numpy().ravel()
                )

            # Only save the individual wav if the layer optimization succeeded
            if synth_success or OVERRIDE_SAVE:
                wavfile.write(
                    layer_filepath + "_synth.wav", 20000, xadv[i].cpu().numpy().ravel()
                )
...
def main(raw_args=None):
    ...
    parser.add_argument(
        ...
        metavar="--A",
        type=str,
        default="psychophysics_wsj400_jsintest",
