...
def get_metamer_list_from_network_name(
    network_name, all_model_info, network_type="image"
):
    """
    ...
    metamer_distances_location = os.path.join(
        model_info["location"],
...
def get_1000_way_class_label_400_16_class_imagenet_val_images(image_path):
    ...
    image_location_orig = os.path.join(
    ...
    matching_images = glob.glob(
        os.path.join(
            image_location_orig, "%s_*_%s_*.JPEG" % (image_number, image_16_category)
        )
    )
...
def get_network_predictions_from_metamer_list_images(
    model, metamer_list, pytorch_transforms=transforms.ToTensor()
):
    ...
    for metamer_layer in metamer_list.keys():
        ...
        for metamer_path in metamer_list[metamer_layer]:
            ...
            labels_1000_way.append(
                get_1000_way_class_label_400_16_class_imagenet_val_images(metamer_path)
            )
            predictions, _ = model(
                image.cuda(), with_latent=False
            ...
            predictions_16_way.append(
                force_16_choice(
                    np.flip(np.argsort(predictions.ravel(), 0)), CLASS_DICT["ImageNet"]
                )
            )
    ...
    return {
        "labels_16_way_all_layers": labels_16_way_all_layers,
...
def get_network_predictions_from_metamer_list_audio(
    model, metamer_list, pytorch_transforms=audio_transforms.AudioToTensor()
):
    """Given a pytorch model, gets the predictions for the outputs in metamer_list"""
    labels_all_layers = {}
    ...
    for metamer_layer in metamer_list.keys():
        ...
        for metamer_path in metamer_list[metamer_layer]:
            ...
            predictions_all.append(
                word_and_speaker_encodings["word_idx_to_word"][np.argmax(predictions)]
            )
...
def evaluate_predicted_vs_actual(labels, predictions):
    ...
    for layer in labels.keys():
        correct_incorrect[layer] = [
            labels[layer][p] == predictions[layer][p]
            for p in range(len(predictions[layer]))
        ]
        average_correct[layer] = np.mean(correct_incorrect[layer])
    return correct_incorrect, average_correct


def bootstrap_imagenet_correct_incorrect(
    correct_incorrect_1000, correct_incorrect_16, num_bootstraps=1000
):
    """
    ...
    for layer in correct_incorrect_1000.keys():
        ...
        for i in range(num_bootstraps):
            ...
            bootstrapped_means_1000[layer].append(
                np.mean(np.array(correct_incorrect_1000[layer])[tmp_idx])
            )
            bootstrapped_means_16[layer].append(
                np.mean(np.array(correct_incorrect_16[layer])[tmp_idx])
            )
...
def bootstrap_correct_incorrect(correct_incorrect, num_bootstraps=1000):
    ...
    for layer in correct_incorrect.keys():
        ...
        for i in range(num_bootstraps):
            tmp_idx = np.random.choice(num_stimuli, num_stimuli)
            bootstrapped_means[layer].append(
                np.mean(np.array(correct_incorrect[layer])[tmp_idx])
            )
