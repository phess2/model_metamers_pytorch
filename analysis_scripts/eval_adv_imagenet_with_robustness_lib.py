...
parser = argparse.ArgumentParser(
    description="Input parameters for evaluating model on adversarial robustness"
)
parser.add_argument(
    "-R",
    "--RANDOMSEED",
...
if args.ATTACK_EPS_LIST is not None:
    ...
    eps_list = [
        convert_string_frac_to_float(e) for e in args.ATTACK_EPS_LIST.split(",")
    ]
    print(eps_list)
else:
    num_eps_steps = int((args.MAX_EPS - args.MIN_EPS) * args.NUM_EPS_STEPS_BETWEEN + 1)
    eps_list = np.concatenate(
        [[0], np.logspace(args.MIN_EPS, args.MAX_EPS, num_eps_steps)]
    )
...
if args.ATTACK_EPS_LIST is None:
    ...
    base_filepath = (
        "adversarial_eval_robustness_lib/rs%d_nsamp%d_maxiter%d_type%s_epsdiv%d_steps%d"
        % (
            RANDOMSEED,
            NUMSAMPLES,
            MAXITER,
            str(attack_norm),
            EPS_DIVIDE,
            args.NUM_EPS_STEPS_BETWEEN,
        )
    )
else:
    # Make a directory for saving:
    base_filepath = (
        "adversarial_eval_robustness_lib/rs%d_nsamp%d_maxiter%d_type%s_epsdiv%d_eps%s"
        % (
            RANDOMSEED,
            NUMSAMPLES,
            MAXITER,
            str(attack_norm),
            EPS_DIVIDE,
            str(args.ATTACK_EPS_LIST).replace("/", "|"),
        )
    )
print(base_filepath)
try:
...
if os.path.isfile(pckl_path) and not args.OVERWRITE_PICKLE:
    raise FileExistsError(
        "The file %s already exists, and you are not forcing overwriting" % pckl_path
    )
...
for eps in eps_list:
    ...
    eval_args = defaults.check_and_fill_args(
        eval_args, defaults.TRAINING_ARGS, ImageNet
    )
