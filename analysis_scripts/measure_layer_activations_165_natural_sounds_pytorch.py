...
for wav_idx, wav_data in enumerate(sound_list):
    test_audio, SR = load_audio_wav_resample(
        os.path.join(wavs_location, wav_data[0].decode("utf-8")),
        DUR_SECS=MEASURE_DUR,
        resample_SR=SR,
    )
...
all_layers = [
    e.split("_fake")[0] for e in all_layers
]  # Don't duplicate these since we aren't synthesizing
new_all_layers = []
...
net_h5py_file_full = h5py.File(
    os.path.join(save_features_dir, filename + "_full.h5"), "w"
)

# Save the list of layers to the hdf5
net_h5py_file["layer_list"] = np.array([layer.encode("utf-8") for layer in all_layers])
net_h5py_file_full["layer_list"] = np.array(
    [layer.encode("utf-8") for layer in all_layers]
)

for sound_idx, sound_info in enumerate(sound_list):
    sound = preproc_sound_np(wav_array[sound_idx, :])
    with torch.no_grad():
        (predictions, rep, layer_returns), orig_image = model(
            sound, with_latent=True
    ...
    if sound_idx == 0:
        for layer in all_layers:
            ...
            if len(layer_shape_165) == 4:
                layer_shape_unraveled = (
                    layer_shape_165[1] * layer_shape_165[2]
                )  # don't take the time dimension into account
            else:
                layer_shape_unraveled = layer_shape_165[1]
            net_layer_dict_full[layer] = net_h5py_file_full.create_dataset(
                layer, (165, layer_shape_full), dtype="float32"
            )
            net_layer_dict[layer] = net_h5py_file.create_dataset(
                layer, (165, layer_shape_unraveled), dtype="float32"
            )
    ...
    for layer_idx, layer in enumerate(all_layers):
        ...
        if layer_returns[layer].ndim == 4:  # NCHW (W is time)
            net_layer_dict[layer][sound_idx, :] = np.mean(
                layer_returns[layer].cpu().detach().numpy(), 3
            ).ravel()
        else:  # fully connected layers do not have a temporal component.
            net_layer_dict[layer][sound_idx, :] = (
                layer_returns[layer].cpu().detach().numpy().ravel()
            )
        net_layer_dict_full[layer][sound_idx, :] = (
            layer_returns[layer].cpu().detach().numpy().ravel()
        )
net_h5py_file.close()
