    ...
    *Internal Function* (called directly from train_model)
    ...
    if args.warm_up_lr:
        schedule = GradualWarmupScheduler(
            optimizer, multiplier=1, total_epoch=1, after_scheduler=schedule
        )
...
def eval_model(args, model, loader, store, adv_only=False):
    ...
    if not adv_only:
        prec1, nat_loss = _model_loop(
            args, "val", loader, model, None, 0, False, writer, store
        )
    else:
    ...
    if args.adv_eval:
        ...
        adv_prec1, adv_loss = _model_loop(
            args, "val", loader, model, None, 0, True, writer, store
        )
    log_info = {
        "epoch": 0,
        "nat_prec1": prec1,
...
def train_model(
    args, model, loaders, *, checkpoint=None, store=None, update_params=None
):
    """
        ...
        if should_log or last_epoch or should_save_ckpt:
            ...
            if isinstance(prec1, dict) or isinstance(adv_prec1, dict):
                if isinstance(prec1, dict):
                    prec1 = "|".join(
                        ["%s:%s" % (p_key, p_value) for p_key, p_value in prec1.items()]
                    )
                if isinstance(adv_prec1, dict):
                    adv_prec1 = "|".join(
                        [
                            "%s:%s" % (p_key, p_value)
                            for p_key, p_value in adv_prec1.items()
                        ]
                    )
                ...
                train_prec1 = "|".join(
                    [
                        "%s:%s" % (p_key, p_value)
                        for p_key, p_value in train_prec1.items()
                    ]
                )
...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer, store):
    ...
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    if multi_task_loss:
        top1 = {
            task_key: AverageMeter()
            for task_key in args.custom_train_loss.all_loss_weights.keys()
        }
        top5 = {
            task_key: AverageMeter()
            for task_key in args.custom_train_loss.all_loss_weights.keys()
    ...
    if adv:
        eps = (
            calc_fadein_eps(epoch, args.eps_fadein_epochs, args.eps)
            if is_train
            else args.eps
        )
    ...
    for i, (inp, target) in iterator:
        ...
        if isinstance(model_logits, dict):
            ...
            for task_key in multi_task_keys:
                ...
                if len(print_target[task_key].shape) == 1:
                    ...
                    prec1, prec5 = helpers.accuracy(
                        model_logits[task_key], print_target[task_key], topk=(1, maxk)
                    )
        ...
        if isinstance(top1_acc, dict):
            task_string = [
                "{task_key} | {0}1 {top1_acc:.3f} | {0}5 {top5_acc:.3f} |".format(
                    prec,
                    top1_acc=top1_acc[task_key],
                    top5_acc=top5_acc[task_key],
                    task_key=task_key,
                )
                for task_key in top1_acc.keys()
            ]
            desc = (
                "{2} Epoch:{0} | Loss {loss.avg:.4f} | {1} | Reg term: {reg} |".format(
                    epoch, "".join(task_string), loop_msg, loss=losses, reg=reg_term
                )
            )
        else:
            desc = (
        ...
        for d, v in zip(descs, vals):
            if isinstance(v, dict):
                for v_key, v_value in v.items():
                    writer.add_scalar(
                        "_".join([prec_type, loop_type, d, v_key]), v_value.avg, epoch
                    )
            else:
    ...
    if isinstance(top1, dict):
        return {
            top1_key: top1_value.avg for top1_key, top1_value in top1.items()
        }, losses.avg
    else:
