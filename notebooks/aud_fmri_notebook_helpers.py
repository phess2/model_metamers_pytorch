...
def split_roi_data_by_participant(voxel_data, voxel_meta, roi_masks):
    ...
    for participant_id in participant_ids:
        ...
        all_participants[-1]["all_aud_voxels"] = voxel_data[
            :, subj_idx == participant_id, :
        ]
    return participant_ids, all_participants


def load_all_voxel_predictions(
    all_networks_and_layers, model_name, spectemp_filter_layer="avgpool"
):
    """
    ...
    neural_location = os.path.join(
        all_networks_and_layers[model_name]["location"],
        all_networks_and_layers[model_name]["neural_regression_folder"],
    )
    neural_analysis_string = all_networks_and_layers[model_name][
        "neural_analysis_string"
    ]
    all_r2 = {}
    all_alphas = {}
...
def responses_network_by_layer(all_participants_answers, experiment_params):
    ...
    for p_idx, participant in enumerate(participants):
        for n_idx, network in enumerate(models):
            model_layers = all_model_info.ALL_NETWORKS_AND_LAYERS_AUDIO[network][
                "layers"
            ]
            ...
            for l_idx, layer in enumerate(model_layers):
                ...
                saved_condition_name = "%d_%s/%d_%s" % (
                    n_idx + 1,
                    network,
                    l_idx,
                    layer,
                ...
                experiment_response_dict[network][layer].append(
                    np.mean(
                        all_participants_answers[participant][saved_condition_name][
                            "correct_incorrect"
                        ]
                    )
                )
        ...
        for model in experiment.keys():
            if model not in list(combined_experiment_dict.keys()):
                ...
                for layer in combined_experiment_dict[model].keys():
                    combined_experiment_dict[model][layer] = (
                        combined_experiment_dict[model][layer]
...
def choose_metamer_layer(metamer_responses_by_layer, measured_layer, return_type):
    ...
    if return_type == "average":
        average_metamer_recognition = np.mean(
            all_human_responses
        )  # This value should vary between 0-1
        return average_metamer_recognition
    elif return_type == "sem":
        sem_metamer_recognition = np.std(all_human_responses) / np.sqrt(
            len(all_human_responses)
        )
...
def load_rsa_values(model_name, all_networks_and_layers, roi_name=None):
    ...
    if (roi_name is None) or (roi_name == "all_aud_voxels"):
        all_values = pickle.load(
            open(os.path.join(rsa_base_path, "all_dataset_rsa_dict.pckl"), "rb")
        )["NH2015"]["trained"][model_name]
    else:
        all_values = pickle.load(
            open(os.path.join(rsa_base_path, "all_dataset_roi_rsa_dict.pckl"), "rb")
        )["NH2015"][roi_name]["trained"][model_name]
    return all_values, all_conv_layers


def plot_roi_analysis_for_models(
    models_to_plot,
    model_colors,
    model_style_dict,
    ...
    for model_idx, model_name in enumerate(models_to_plot):
        ...
        if analysis_type == "regression":
            all_r2, all_alphas, all_voxel_info, all_conv_layers = (
                load_all_voxel_predictions(all_networks_and_layers, model_name)
            )
            if not all_voxel_info:
                print("### Missing all values for model %s ###" % model_name)
                all_model_results.append(
                    [
                        np.ones(len(all_participants)) * np.nan
                ...
                all_metamer_best_layer.append(
                    [np.nan for roi_idx in range(len(roi_names))]
                )
                all_metamer_best_layer_sem.append(
                    [np.nan for roi_idx in range(len(roi_names))]
                )
                all_model_results_all_layers.append(
                    [
                        np.ones(len(all_participants), len(all_conv_layers)) * np.nan
                        for roi_idx in range(len(roi_names))
                    ]
                )
                continue
        ...
        for roi_idx, roi_name in enumerate(roi_names):
            ...
            if analysis_type == "regression":
                participant_ids, all_participants = split_roi_data_by_participant(
                    all_r2, all_voxel_info["voxel_meta"], roi_masks
                )

                all_participants_median_layer_data = []
                all_participants_median_layer_data_mean_subtracted = []

                for participant_idx, participant_all_data in enumerate(
                    all_participants
                ):
                    ...
                    median_layer_data_mean_subtracted = median_layer_data - np.nanmean(
                        median_layer_data
                    )

                    all_participants_median_layer_data.append(median_layer_data)
                    all_participants_median_layer_data_mean_subtracted.append(
                        median_layer_data_mean_subtracted
                    )

                    num_voxels[roi_name] += median_layer_voxel_data.shape[1]

                all_participants_median_layer_data = np.array(
                    all_participants_median_layer_data
                )
                ...
                for participant_idx, participant_all_data in enumerate(
                    ...
                    participants_to_measure.pop(
                        participant_idx
                    )  # remove the participant we are holding out
                    ...
                    average_layer_data = np.nanmean(
                        all_participants_median_layer_data[participants_to_measure, :],
                        0,
                    )
                    best_layer = np.argmax(average_layer_data)
                    participant_roi_from_held_out.append(
                        np.nanmean(
                            all_participants_median_layer_data[
                                participant_idx, best_layer
                            ]
                        )
                    )
                    participant_best_layer_roi_from_held_out.append(best_layer)

                    participant_all_layer_data.append(
                        all_participants_median_layer_data[participant_idx, :]
                    )

            elif analysis_type == "rsa":
                all_values, all_conv_layers = load_rsa_values(
                    model_name, all_networks_and_layers, roi_name=roi_name
                )
                participant_roi_from_held_out = []
                participant_best_layer_roi_from_held_out = []
                participant_all_layer_data = []
                ...
                for participant_idx, participant in enumerate(all_values.keys()):
                    ...
                    median_best_value = all_values[participant][
                        "best_layer_rsa_median_across_splits"
                    ]
                    participant_roi_from_held_out.append(median_best_value)

                    best_layer = all_values[participant]["splits_median_best_layer_idx"]

                    cross_val_neural = all_values[participant][
                        "leave_one_out_neural_r_cv_test_splits"
                    ]
                    if model_idx == 0:
                        all_participants_noise_ceiling[roi_name].append(
                            np.median(cross_val_neural)
                        )

                    participant_best_layer_roi_from_held_out.append(best_layer)

                    participant_all_layer_data.append(
                        all_values[participant]["splits_median_distance_all_layers"]
                    )
        ...
        median_best_layer_name = [
            all_conv_layers[np.squeeze(int(m))] for m in median_best_layer
        ]
        save_best_layer_names[model_name] = {
            roi_name: median_best_layer_name[roi_idx]
            for roi_idx, roi_name in enumerate(roi_names)
        }

        average_metamer_recognition = [
            choose_metamer_layer(combined_experiment_dict[model_name], m, "average")
            for m in median_best_layer_name
        ]
        sem_metamer_recognition = [
            choose_metamer_layer(combined_experiment_dict[model_name], m, "sem")
            for m in median_best_layer_name
        ]
        ...
        average_all_metamer_recognition = [
            choose_metamer_layer(combined_experiment_dict[model_name], m, "average")
            for m in all_conv_layers
        ]
        sem_all_metamer_recognition = [
            choose_metamer_layer(combined_experiment_dict[model_name], m, "sem")
            for m in all_conv_layers
        ]
    ...
    for roi_plot_idx, roi_name in enumerate(roi_names):
        ...
        for model_idx, model_name in enumerate(models_to_plot):
            score_val = np.array(all_model_results)[model_idx, roi_plot_idx, :].mean(
                axis=0
            )
            score_sem = (
                np.array(all_model_results)[model_idx, roi_plot_idx, :].std(axis=0)
            ) / np.sqrt(num_participants)
        ...
        rho_val, p_val_rho_raw = scipy.stats.spearmanr(
            np.array(all_scores_list), np.array(all_metamers_list)
        )
        p_val_rho = np.min(
            [p_val_rho_raw * bonferroni_correction_factor, 1.0]
        )  # Max out at 1.

        r_val, p_val_r_raw = scipy.stats.pearsonr(
            np.array(all_scores_list), np.array(all_metamers_list)
        )
        p_val_r = np.min([p_val_r_raw * bonferroni_correction_factor, 1.0])

        plt.text(
            0.01,
            0.25,
            "rho=%0.3f, p=%0.2f, (raw p=%0.2f)" % (rho_val, p_val_rho, p_val_rho_raw),
        )
        plt.text(
            0.01,
            0.15,
            "R^2=%0.3f, p=%0.2f, (raw p=%0.2f)" % (r_val**2, p_val_r, p_val_r_raw),
        )
