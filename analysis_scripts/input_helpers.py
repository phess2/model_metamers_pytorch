...
def generate_import_audio_functions(
    audio_func="psychophysicskell2018dry", preproc_scaled=1, rms_normalize=1, **kwargs
):
    """
    ...
    if audio_func == "psychophysicskell2018dry_overlap_jsinv3":
        return partial(
            psychophysicskell2018dry_overlap_jsinv3,
            preproc_scaled=preproc_scaled,
            rms_normalize=rms_normalize,
            **kwargs,
        )
    elif audio_func == "psychophysics_wsj400_jsintest":
        return partial(
            psychophysics_wsj400_jsintest,
            preproc_scaled=preproc_scaled,
            rms_normalize=rms_normalize,
            **kwargs,
        )
        ...
        return partial(
            use_audio_path_specified_audio,
            preproc_scaled=preproc_scaled,
            rms_normalize=rms_normalize,
            **kwargs,
        )
...
def psychophysicskell2018dry_overlap_jsinv3(
    WAV_IDX, preproc_scaled=1, rms_normalize=None, SR=20000
):
    """
    ...
    word_to_int = dict(
        zip(
            behavioral_dataset_kell["stimuli"]["word"],
            behavioral_dataset_kell["stimuli"]["word_int"],
        )
    )
    int_to_word = dict(
        zip(
...
def psychophysics_wsj400_jsintest(
    WAV_IDX, preproc_scaled=1, rms_normalize=None, SR=20000
):
    """
    ...
    pckl_path = os.path.join(
        ASSETS_PATH,
        "word_WSJ_validation_jsin_400words_1samplesperword_with_metadata.pckl",
    )
...
def use_audio_path_specified_audio(
    WAV_IDX,
    wav_path=None,
    wav_word=None,
    preproc_scaled=1,
    ...
    word_and_speaker_encodings = pickle.load(
        open(WORD_AND_SPEAKER_ENCODINGS_PATH, "rb")
    )
...
def generate_import_image_functions(
    image_func="small_16_class_imagenet",
    im_shape=224,
    image_path=None,
    image_class=None,
    data_format="NHWC",
    ...
    if image_func == "256_16_class_imagenet":
        return partial(
            medium_256_16_class_imagenet, im_shape=im_shape, data_format=data_format
        )
    elif image_func == "400_16_class_imagenet_val":
        return partial(
            medium_400_16_class_imagenet_val_images,
            im_shape=im_shape,
            data_format=data_format,
        )
    elif image_func == "load_specified_image_path":
        return partial(
            use_image_path_specified_image,
        ...
        return partial(
            use_pytorch_datasets,
            data_format=data_format,
            im_shape=im_shape,
            **pytorch_dataset_kwargs,
        )
...
def medium_256_16_class_imagenet(IMG_IDX, im_shape=224, data_format="NHWC"):
    ...
    mage_name = glob.glob("%s/%d_*" % (image_locations, IMG_IDX))[0].split("/")[
        -1
    ]  # image_list[IMG_IDX]
...
def medium_400_16_class_imagenet_val_images(IMG_IDX, im_shape=224, data_format="NHWC"):
    ...
    assert len(glob.glob("%s/*.JPEG" % image_locations)) == 400, (
        "Did not find exactly 400 images in %s" % image_locations
    )
    image_name = glob.glob("%s/%d_*" % (image_locations, IMG_IDX))[0].split("/")[
        -1
    ]  # image_list[IMG_IDX]
    assert int(image_name.split("_")[0]) == IMG_IDX, "Check the ordering for the images"
...
def make_400_16_class_imagenet_val_data():
    ...
    from analysis_scripts.imagenet_16_categories_validation_paths import (
        validation_paths,
    )
    ...
    output_image_path = os.path.join(
        ASSETS_PATH, "full_400_16_class_imagenet_val_images/"
    )
    small_dataset_idx = 0
    ...
    do_not_include_val_list = [
        "ilsvrc/val/n02690373/ILSVRC2012_val_00026189.JPEG",  # includes truck
    ...
    for image_class_idx, image_class in enumerate(image_classes):
        ...
        while class_total < total_per_class:
            ...
            image_path_dict[image_class].append(
                random_image.split("images_complete/")[-1]
            )
            copyfile(
                os.path.join(image_net_path, random_image),
                os.path.join(
                    output_image_path,
                    "%d_%d_%s_%s_%s.JPEG"
                    % (
        ...
        with open(
            os.path.join(
                output_image_path,
                "image_list_full_400_16_class_imagenet_val_images.json",
            ),
            "w",
        ) as outfile:
...
def make_256_16_class_imagenet():
    ...
    image_list_path = os.path.join(
        ASSETS_PATH, "image_names_16_class_imagenet_training_data"
    )
    ...
    for image_class_idx, image_class in enumerate(image_classes):
        all_image = (
            open(os.path.join(image_list_path, "%s.txt" % image_class))
            .read()
            .splitlines()
        )
        random_images_perm = np.random.permutation(range(len(all_image)))[
            10:26
        ]  # grab 16 of each class, so they do not overlap with a precious set.
        ...
        for random_image in random_images_names:
            ...
            copyfile(
                os.path.join(image_net_path, class_name, random_image),
                os.path.join(
                    output_image_path,
                    "%d_%d_%s_%s"
                    % (small_dataset_idx, image_class_idx, image_class, random_image),
                ),
            )
            small_dataset_idx += 1


def use_image_path_specified_image(
    IMG_IDX, image_path=None, image_class=None, im_shape=224, data_format="NHWC"
):
...
def use_pytorch_datasets(
    IMG_IDX, DATA, train_or_val="val", im_shape=224, data_format="NHWC"
):
    print("Using pytorch dataset %s" % DATA)
    if im_shape != 224:
        raise NotImplementedError(
            "Pytorch datasets not implemented for arbitrary shapes yet"
        )
    ...
    if train_or_val == "train":
        raise NotImplementedError(
            "train subset is not implemented for pytorch datasets"
        )
    elif train_or_val == "val":
        only_val = True
    else:
        raise ValueError(
            "train_or_val must be 'train' or 'val', currently set as %s" % train_or_val
        )
...
def get_multiple_samples_pytorch_datasets(
    NUM_EXAMPLES,
    DATA,
    train_or_val="val",
    im_shape=224,
