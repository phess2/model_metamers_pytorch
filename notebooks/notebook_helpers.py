...
def display_all_visual_models_for_experiment(
    exp_name,
    example_idx,
):
    ...
    experiment_name = all_model_info.TURK_IMAGE_EXPERIMENTS_GROUPINGS[
        "experiment_%d" % exp_num
    ]["paper_experiment_name"]
    jsin_configs = all_model_info.TURK_IMAGE_EXPERIMENTS_GROUPINGS[
        "experiment_%d" % exp_num
    ]["experiment_params_web"]

    experiment_folder_web = all_model_info.TURK_IMAGE_EXPERIMENTS_GROUPINGS[
        "experiment_%d" % exp_num
    ...
    orig_html_path = experiment_folder_web + "/%d_%s/%d_%s/%d_%s_%s.png" % (
        0,
        "orig",
        0,
        "orig",
    ...
    try:
        ...
        if "hmax_standard" in plot_model_order:
            plt.imshow(
                plt.imread(metamer_png_path), interpolation="none", cmap="Greys_r"
            )
        else:
        ...
        print(
            "Missing %s, this image was not included in this experiment. Try another example."
            % orig_html_path
        )
    ...
    for model_idx, model in enumerate(plot_model_order):
        ...
        for plot_idx, layer in enumerate(plot_layers):
            try:
                metamer_html_path = model_folder_web + "/%d_%s/%d_%s_%s.png" % (
                    plot_idx,
                    layer,
                    example_idx,
                    model,
                ...
                ax = plt.subplot(
                    1 + len(plot_model_order),
                    max_num_plots,
                    1 + (max_num_plots * (model_idx + 1)) + plot_idx,
                )
                if "hmax" in model:
                    plt.imshow(
                        plt.imread(metamer_png_path),
                        interpolation="none",
                        cmap="Greys_r",
                    )
                else:
                ...
                plt.title(
                    layer.split("_fake_relu")[0].split("_fakerelu")[0], fontsize=16
                )
...
def display_all_audio_models_for_experiment(
    exp_name,
    example_idx,
):
    exp_num = audio_experiment_dict_by_name[exp_name]

    experiment_name = all_model_info.TURK_AUDIO_EXPERIMENTS_GROUPINGS[
        "experiment_%d" % exp_num
    ]["paper_experiment_name"]
    jsin_configs = all_model_info.TURK_AUDIO_EXPERIMENTS_GROUPINGS[
        "experiment_%d" % exp_num
    ]["experiment_params_web"]

    experiment_folder_web = all_model_info.TURK_AUDIO_EXPERIMENTS_GROUPINGS[
        "experiment_%d" % exp_num
    ]["experiment_folder_web"]
    ...
    max_num_plots = 10
    orig_html_path = experiment_folder_web + "/%d_%s/%d_%s/%d_%s_%s.wav" % (
        0,
        "orig",
        0,
        "orig",
    ...
    try:
        ...
        load_audio = False
        print(
            "Missing %s, this audio was not included in this experiment. Try another example."
            % orig_html_path
        )
    ...
    for model_idx, model in enumerate(plot_model_order):
        ...
        for plot_idx, layer in enumerate(plot_layers):
            metamer_html_path = model_folder_web + "/%d_%s/%d_%s_%s.wav" % (
                plot_idx,
                layer,
                example_idx,
                model,
...
def responses_network_by_layer_mat(
    all_subjects_answers, model_layers, conditions, experiment_params
):
    network_response_dict = {}
    condition_idx = list(conditions).index("Human  ")

    for l_idx, layer in enumerate(model_layers):
        network_response_dict[layer.strip()] = list(
            all_subjects_answers[:, l_idx, condition_idx]
        )
...
def unpack_experiment_mat(mat_file_path):
    ...
    participants = mat_contents[
        "participants"
    ]  # participant IDs were removed for public release
