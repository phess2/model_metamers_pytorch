...
from .timm_transformer_helpers.constants import (
    IMAGENET_DEFAULT_MEAN,
    IMAGENET_DEFAULT_STD,
)
from .timm_transformer_helpers.models.helpers import (
    build_model_with_cfg,
    checkpoint_seq,
)
from .timm_transformer_helpers.models.layers import (
    DropBlock2d,
    DropPath,
    AvgPool2dSame,
    BlurPool2d,
...
class ResNet(nn.Module):
    ...
    def forward(self, x, with_latent=False, no_relu=False, fake_relu=False):
        del no_relu
        if (
            not self.is_relu_act
        ):  # Make sure that fake_relu is false if we don't have relus
            fake_relu = False
        if with_latent:
            all_outputs = {"input_after_preproc": x}
            x, all_outputs_features = self.forward_features(
