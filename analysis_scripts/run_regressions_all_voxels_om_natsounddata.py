...
parser.add_argument(
    "LAYER", metavar="L", type=int, help="index into the list of possible layers"
)
...
parser.add_argument(
    "-Z", "--ZERO_CENTER", action="store_true", help="If true, zero mean the targets"
)
...
possible_alphas = possible_alphas[
    ::-1
]  # reverse the order to make it consistant with Kell2018's conventions
...
if zero_center:
    save_path = os.path.join(
        EXPORT_DIR_FULL,
        "voxelwise_regression_%s_randseed%d_zero_center_fmri_activations.pckl"
        % (layer, rand_seed),
    )
else:
    save_path = os.path.join(
        EXPORT_DIR_FULL, "voxelwise_regression_%s_randseed%d.pckl" % (layer, rand_seed)
    )
...
if sys.version[0] == "2":
    ...
    roi_masks = pickle.load(
        open(os.path.join(fMRI_DATA_PATH, "roi_masks.cpy"), "rb"), encoding="latin1"
    )
...
for split_idx in range(num_random_splits):
    ...
    for voxel_idx in range(voxel_data.shape[1]):
        print(voxel_idx)
        r2_voxels[voxel_idx, split_idx], alphas[voxel_idx, split_idx] = (
            runRidgeWithCorrectedR2_ThreeRunSplit(
                features,
                voxel_data,
                voxel_idx,
                is_train_data,
