...
layer_order, metamer_list = get_metamer_list_from_network_name(
    args.MET_NET, all_model_info, network_type="audio"
)

prediction_dict = get_network_predictions_from_metamer_list_audio(model, metamer_list)

correct_incorrect, average_correct = evaluate_predicted_vs_actual(
    prediction_dict["labels_all_layers"], prediction_dict["predictions_all_layers"]
)
