...
def make_and_restore_model(
    *_,
    ...
    """
    classifier_model = (
        dataset.get_model(arch, pytorch_pretrained, arch_kwargs)
    ...
    if resume_path:
        if os.path.isfile(resume_path):
            ...
            try:
                ...
                print(
                    "Missing state dict key %s from checkpoint. Assuming checkpoint is simply the state dictionary and we do not need a key"
                    % state_dict_path
                )
                sd = checkpoint
            ...
            if append_name_front_keys is not None:
                ...
                for key_idx in range(len(append_name_front_keys)):
                    sd_temp = {
                        "%s%s" % (append_name_front_keys[key_idx], k): v
                        for k, v in sd.items()
                    }
            ...
            for old_key, new_key in remap_checkpoint_keys.items():
                ...
                if (
                    type(new_key) is list
                ):  # If there are multiple keys that should be the same value (ie with attacker model)
            ...
            for old_prefix, new_prefix in change_prefix_checkpoint.items():
                ...
                for sd_key in sd_keys_temp:
                    if (
                        type(old_prefix) == int
                    ):  # If we need to add prefix to EVERYTHING
            ...
            print(
                "=> loaded checkpoint '{}' (epoch {})".format(
                    resume_path, checkpoint.get("epoch", "epoch number not found")
                )
            )
        else:
