...
class InversionLossLayer(torch.nn.Module):
    ...
    def forward(self, model, inp, targ):
        ...
        rep = (
            all_outputs[self.layer_to_invert]
            .contiguous()
            .view(all_outputs[self.layer_to_invert].size(0), -1)
        )
...
class InversionLossLayerWithRandomSingleUnitOptimizationDropout(torch.nn.Module):
    ...
    def __init__(
        ...
        super(
            InversionLossLayerWithRandomSingleUnitOptimizationDropout, self
        ).__init__()
        self.layer_to_invert = layer_to_invert
        self.fake_relu = fake_relu
        self.normalize_loss = normalize_loss
    ...
    def forward(self, model, inp, targ):
        ...
        if isinstance(targ, list):
            assert isinstance(rep, list), (
                "targ is list but rep is not. this is not supported."
            )
        else:
            targ = targ.view(rep.shape)

        # Choose which unit to optimize
        if (self.optimization_count % self.optimization_step == 0) and (
            self.optimization_count < self.all_units_count
        ):
            if isinstance(targ, list):
        ...
        if isinstance(targ, list):
            if self.enable_dropout_flag and (
                self.optimization_count < self.all_units_count
            ):
            ...
            (len(targ.shape) == 4)
            and self.enable_dropout_flag
            and (self.optimization_count < self.all_units_count)
        ):
            targ = targ[:, self.selected_unit, :, :]
            rep = rep[:, self.selected_unit, :, :]
        elif (
...
class InversionLossLayerWithCoarseDefineSpecTemp111(torch.nn.Module):
    ...
    def forward(self, model, inp, targ):
        if self.enable_dropout_flag:
            self.optimization_count += 1
            self.cutoff_idx = min(
                floor(self.optimization_count / self.optimization_step),
                len(self.filter_cutoffs) - 1,
            )
            self._enable_dropout_functions()
        else:
            self._disable_dropout_functions()
        ...
        if self.layer_to_invert in model.coarse_define_layers:
            ...
            include_filts = (
                abs(all_freqs[:, 0]) <= self.filter_cutoffs[self.cutoff_idx][0]
