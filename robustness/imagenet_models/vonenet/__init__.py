...
def get_vonenet_model(
    model_arch=None,
    pretrained=True,
    map_location="cpu",
    weightsdir_path=None,
    num_stochastic_copies=None,
    vone_outside_sequential=False,
    **kwargs,
):
    """
    ...
    if pretrained:
        if weightsdir_path is None:
            raise ValueError(
                "The path to the saved pytorch checkpoint is not specified"
            )
