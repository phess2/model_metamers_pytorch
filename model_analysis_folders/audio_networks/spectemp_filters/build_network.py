...
def build_net(
    ...
    ds = jsinV3(
        ...
        eval_max=8,
        **ds_kwargs,
    )  # Sequential will change the state dict names

    # Path to the network checkpoint to load
    resume_path = os.path.join(
        MODEL_BASE_PATH, "audio_networks", "pytorch_checkpoints", "spectemp_model.pt"
    )
    ...
    model, _ = make_and_restore_model(
        arch="spectemp_filts_time_average_coch1",
        dataset=ds,
        parallel=False,
    ...
    model.model.filter_freqs = model.model._modules[
        "1"
    ].spectempfilterbank.spec_temp_freqs
