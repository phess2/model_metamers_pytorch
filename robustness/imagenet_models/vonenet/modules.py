    ...
    def _append_all_outputs(self, all_outputs, all_outputs_new):
        for key in all_outputs_new:
            if key in all_outputs:
                raise ValueError(
                    "Specifying two key names for parts of "
...
class VOneBlock(nn.Module):
    ...
    def noise_f(self, x):
        if self.noise_mode == "neuronal":
            ...
            if self.fixed_noise is not None:
                ...
                ).rsample() * torch.sqrt(F.relu(x.clone()) + eps)
            x -= self.noise_level
            x /= self.noise_scale
            ...
            if self.fixed_noise is not None:
                ...
                x += torch.distributions.normal.Normal(
                    torch.zeros_like(x), scale=self.noise_level
                ).rsample()
        return self.noise(x)
    ...
    def fix_noise(self, batch_size=256, seed=None):
        ...
        if self.noise_mode == "gaussian":
            self.fixed_noise = (
                torch.distributions.normal.Normal(noise_mean, scale=self.noise_level)
                .rsample()
                .to(device)
            )
