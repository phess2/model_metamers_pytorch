...
def build_alexnet_model(weights_url, config, layer_for_fc=None, out_dim_fc=None):
    ...
    if weights_url is not None:
        ...
        checkpoint = load_state_dict_from_url(
            weights_url, model_dir=None, map_location=torch.device("cpu")
        )
        state_dict = {
...
def ipcl1_imagenet_transfer_head(layer_for_fc="fc7"):
    ...
    model, transform = build_alexnet_model(
        weights_url, config, layer_for_fc=layer_for_fc, out_dim_fc=out_dim_fc
    )
...
def ipcl5():
    ...
    filename = (
        "alexnet_gn_dim128_unsupervised_transforms_checkpoint_weights_only.pth.tar"
    )
    weights_url = os.path.join(url_root, "project_instancenet", "ipcl", filename)
