...
def filt_spec_mod(
    ...
    pos_freqs = (
        float(sr_oct) * np.arange(0, nyq_idx + 1) / float(N)
...
def filt_spectemp_mod(
    ...
    if ~np.isnan(fc_cycPoct):
        Hs = filt_spec_mod(
            fc_cycPoct, N_F, sr_oct, LOWPASS_F, HIGHPASS_F, show_plots, zero_pad[1]
        )
    else:
        if zero_pad[1]:
        ...
        for fc_cycPoct in spec_mod_rates:
            ...
            for fc_Hz in temp_mod_rates:
                ...
                if make_plots:
                    ...
                    plt.colorbar()
                    plt.title(
                        "Temp Mod %.1f, Spec Mod %.1f" % (fc_sign * fc_Hz, fc_cycPoct)
                    )
                hts_idx += 1
