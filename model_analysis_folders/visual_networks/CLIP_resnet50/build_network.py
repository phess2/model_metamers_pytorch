...
class CLIPModelWithLabels(torch.nn.Module):
    def __init__(self, clip_model, device="cpu"):
        ...
        if os.path.exists(
            os.path.join(os.path.dirname(SCRIPT_DIR), "zeroshot_weights.pt")
        ):
            ...
            zeroshot_weights = self.zeroshot_classifier(
                clip_model, imagenet_classes, imagenet_templates
            )
            torch.save(
                zeroshot_weights,
                os.path.join(os.path.dirname(SCRIPT_DIR), "zeroshot_weights.pt"),
    ...
    def forward(self, x, with_latent=False, fake_relu=False, no_relu=False):
        if with_latent:
            image_features, _, all_outputs = self.vision_embedding(
                x, with_latent=with_latent, fake_relu=fake_relu, no_relu=no_relu
            )
            all_outputs["image_features"] = (
                image_features  # Do not use logits because the gradient computation through the norm causes problems.
            )
        else:
...
def build_net(ds_kwargs={}, return_metamer_layers=False, dataset_name="ImageNet"):
    ...
    ds.scale_image_save_PIL_factor = (
        255  # Do not scale the output images by 255 when saving with PIL
    )
...
def main(return_metamer_layers=False, ds_kwargs={}, dataset_name="ImageNet"):
    if return_metamer_layers:
        model, ds, metamer_layers = build_net(
            return_metamer_layers=return_metamer_layers, ds_kwargs=ds_kwargs
        )
        return model, ds, metamer_layers
