...
def compute_median_across_predictions(combine_predictions):
    ...
    median_from_best_layers = np.median(
        median_across_splits[arg_layer_max, np.arange(num_voxels)]
    )

    # take the median across voxels, choosing the best layer from above, and take the STD across splits
    median_across_voxels = np.median(
        combine_predictions[arg_layer_max, np.arange(num_voxels), :], 0
    )
...
def ridgeRegressSplits(
    ...
    ridgeCrossVal = sklearn.linear_model.RidgeCV(
        alphas=possible_alphas,
    ...
    if (alpha == possible_alphas[0]) or (alpha == possible_alphas[-1]):
        print(
            "WARNING: BEST ALPHA %.2E IS AT THE EDGE OF THE SEARCH SPACE FOR VOXEL %d"
            % (alpha, voxel_idx)
        )
    ...
    if not np.isclose(np.std(yhat), 0):
        ...
        print(
            "WARNING: VOXEL %d is predicted as only the expected value. Setting correlation to zero."
            % voxel_idx
        )
        r2 = 0
    r2 = r2**2
...
def runRidgeWithCorrectedR2_ThreeRunSplit(
    features,
    voxel_data,
    voxel_idx,
    is_train_data,
    ...
    r2_mean, alpha_mean, yhat_mean = ridgeRegressSplits(
        features,
    ...
    for run in [0, 1, 2]:
        r2, alpha, yhat = ridgeRegressSplits(
            features,
    ...
    for split_idx, split in enumerate([[0, 1], [1, 2], [0, 2]]):
        split_r, split_p = scipy.stats.pearsonr(
            voxel_data[is_train_data, voxel_idx, split[0]],
            voxel_data[is_train_data, voxel_idx, split[1]],
        )
        rvs.append(split_r)

        if (not np.isclose(np.std(split_yhat[split[0]]), 0)) and (
            not np.isclose(np.std(split_yhat[split[1]]), 0)
        ):
            split_r_hat, split_p_hat = scipy.stats.pearsonr(
                split_yhat[split[0]], split_yhat[split[1]]
            )
        else:  # if one of the predictions is constant, then set the r^2 to zero.
            print(
                "WARNING: VOXEL %d, Run %s is predicted as only the expected value. Setting correlation to zero."
                % (voxel_idx, split)
            )
            split_r_hat = 0
    ...
    rv = max(
        3 * rv_med / (1 + 2 * rv_med), 0.182
    )  # Value changed to 0.182 which corresponds to p<0.05 for correlations of two random gaussian variables of length 83 (length of the training data)
    rv_hat_med = np.nanmedian(rv_hats)
    rv_hat = max(
        3 * rv_hat_med / (1 + 2 * rv_hat_med), 0.183
    )  # Value changed to 0.183 which corresponds to p<0.05 for correlations of two random gaussian variables of length 82 (length of the test data)
