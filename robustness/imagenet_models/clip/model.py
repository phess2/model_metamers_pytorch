...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, with_latent=False, all_outputs={}):
        if with_latent:
            for layer_idx, layer_func in enumerate(self.resblocks):
                x = layer_func(x)
                if with_latent:
                    all_outputs["block_%d" % layer_idx] = x.permute(
                        1, 0, 2
                    )  # Neeed to permute this...
            return x, None, all_outputs
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        if with_latent:
            x, _, all_outputs = self.transformer(
                x, with_latent=with_latent, all_outputs=all_outputs
            )
        else:
