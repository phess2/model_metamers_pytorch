...
def load_null_distributions(model_path, null_glob, layers, remap_pckl_keys):
    """
    ...
    all_nulls = {"spearman_rho": {}, "pearson_r": {}, "dB_SNR": {}, "norm_noise": {}}
    for n_idx, null_file in enumerate(null_folder):
        ...
        if n_idx == 0:
            for layer in layers:
                all_nulls["spearman_rho"][layer] = np.array(
                    []
                )  # loaded_null['spearman_r_dict']
                all_nulls["pearson_r"][layer] = np.array(
                    []
                )  # loaded_null['pearson_r_dict']
                all_nulls["dB_SNR"][layer] = np.array(
                    []
                )  # loaded_null['dB_snr_values_dict']
                all_nulls["norm_noise"][layer] = np.array(
                    []
                )  # loaded_null['norm_noise_dict']
        for layer in layers:
            all_nulls["spearman_rho"][layer] = np.concatenate(
                [
                    all_nulls["spearman_rho"][layer],
                    loaded_null["spearman_r_dict"][layer],
                ],
                0,
            )
            all_nulls["pearson_r"][layer] = np.concatenate(
                [all_nulls["pearson_r"][layer], loaded_null["pearson_r_dict"][layer]], 0
            )
            all_nulls["dB_SNR"][layer] = np.concatenate(
                [all_nulls["dB_SNR"][layer], loaded_null["dB_snr_values_dict"][layer]],
                0,
            )
            all_nulls["norm_noise"][layer] = np.concatenate(
                [all_nulls["norm_noise"][layer], loaded_null["norm_noise_dict"][layer]],
                0,
            )
...
def remap_null_pckl_keys(loaded_null_pckl, remap_pckl_keys):
    """
    ...
    for saved_value in list(loaded_null_pckl.keys()):
        for remap_key in list(remap_pckl_keys.keys()):
            loaded_null_pckl[saved_value][remap_pckl_keys[remap_key]] = (
                loaded_null_pckl[saved_value][remap_key]
            )

    return loaded_null_pckl


def get_metamer_distances_wsn_word(model_path, metamer_path, layers, metamer_subset):
    """
    ...
    all_pckl_paths = glob.glob(
        os.path.join(model_path, metamer_path, "*_SOUND_*", "all_metamers_pickle.pckl")
    )
    ...
    for m_idx, metamer_pckl in enumerate(all_pckl_paths):
        ...
        if metamer_subset is not None:
            if (
                len([x for x in metamer_subset if "/%d_SOUND_" % x in metamer_pckl])
                != 1
            ):
                continue
        with open(metamer_pckl, "rb") as f:
            ...
            layer_correct_tmp = {}
            orig_prediction = np.argmax(
                loaded_metamer_pckl["predictions_orig"].cpu().detach().numpy()
            )
            for metamer_layer in layers:
                if m_idx == 0:
                    shape_activations_each_layer[metamer_layer] = (
                        loaded_metamer_pckl["all_outputs_orig"][metamer_layer]
                        .cpu()
                        .detach()
                        .numpy()
                        .shape
                ...
                synth_prediction = np.argmax(
                    loaded_metamer_pckl["predictions_out_dict"][metamer_layer]
                    .cpu()
                    .detach()
                    .numpy()
                )
                ...
                null_overlap_glob = glob.glob(
                    os.path.join(
                        synth_path_dir,
                        "null_overlap_images",
                        "*_%s_synth.wav" % metamer_layer,
                    )
                )
                if (
                    len(null_overlap_glob) == 1
                ):  ## Say that the label isn't matching if we didn't pass null.
                    layer_correct_tmp[metamer_layer] = False
                assert len(null_overlap_glob) <= 1, (
                    "Found multiple null overlap images for %s" % synth_path_dir
                )
                ...
                if not (layer_correct_tmp[metamer_layer]) or (
                    len(null_overlap_glob) == 1
                ):
                    images_that_didnt_converge.append(
                        [
                            loaded_metamer_pckl["xadv_dict"][metamer_layer],
                    ...
                    synth_list = glob.glob(
                        os.path.join(synth_path_dir, "*_%s_synth.wav" % metamer_layer)
                    )
                    len_assertion = (
                        "Found %d synth.wav matching the layer when expecting 1"
                        % len(synth_list)
                    )
    ...
    return_dict = {
        "distance_measures_dict": distance_measures_dict,
...
def get_metamer_distances_imagenet(
    model_path,
    metamer_path,
    layers,
    metamer_subset,
    ...
    for m_idx, metamer_pckl in enumerate(all_pckl_paths):
        ...
        with open(metamer_pckl, "rb") as f:
            ...
            for metamer_layer in layers:
                ...
                # that were marked as not passing the null check
                if not (layer_correct_tmp_16_class[metamer_layer]) or (
                    len(null_overlap_glob) == 1
                ):
                    images_that_didnt_converge.append(
                        [
                    ...
                    synth_list = glob.glob(
                        os.path.join(synth_path_dir, "*_%s_synth.png" % metamer_layer)
                    )
                    len_assertion = (
                        "Found %d synth.png matching the layer when expecting 1"
                        % len(synth_list)
                    )
                    assert len(synth_list) == 1, len_assertion
                    synth_images[metamer_layer].append(synth_list[0])
...
def print_null_dist(
    null, metamer, min_max_check_null_overlap="max", return_overlap_images_idx=False
):
    """
    ...
    if min_max_check_null_overlap == "max":
        ...
        print(
            "layer name & number null & number metamers & null max & metamer min \\\\"
        )
        for i, layer in enumerate(null.keys()):
            if np.max(null[layer][:,]) > np.min(metamer[layer][:]):
                ...
                print(
                    [
                        "%d:%f\n" % (idx, metamer[layer][idx])
                        for idx in range(len(metamer[layer][:]))
                        if (metamer[layer][idx] < np.max(null[layer][:,]))
                    ]
                )
                if return_overlap_images_idx:
                    overlap_images_idx[layer] = np.argwhere(
                        metamer[layer][:] < np.max(null[layer][:,])
                    )
                print(
                    layer,
                    " & ",
        ...
        print("Distance between null and metamers")
        print(
            "layer name & number null & number metamers & null min & metamer max \\\\"
        )
        for i, layer in enumerate(null.keys()):
            if np.min(null[layer][:,]) < np.max(metamer[layer][:]):
                ...
                if return_overlap_images_idx:
                    overlap_images_idx[layer] = np.argwhere(
                        metamer[layer][:] > np.min(null[layer][:,])
                    )
                print(
                    layer,
                    " & ",
    ...
    print(
        "layer name & number null & number metamers & null median & metamer median \\\\"
    )
    for i, layer in enumerate(null.keys()):
        print(
            layer,
...
def print_number_of_metamers_passing_criteria_wsn_word(
    all_layer_correct,
        ...
        for metamer_idx, metamer in enumerate(all_layer_correct):
            ...
            if bool(metamer[layer]) and return_metamer_distances:
                distance_list[layer].append(
                    distance_measure_each_metamer[metamer_idx][layer][layer]
                )
...
def print_number_of_metamers_passing_criteria_imagenet(
    all_layer_correct,
    all_layer_correct_16_class,
    return_metamer_distances=True,
    distance_measure_each_metamer=None,
...
def plot_metamer_vs_null(
    ...
    for layer in distances_for_hist.keys():
        ...
        distance_min_value = min(
            -1,
            min(null[layer].ravel()),
            min(np.array(all_metamer_layer_distances).ravel()),
        )
        distance_max_value = max(
            1,
            max(null[layer].ravel()),
    ...
    for metamer_idx, metamer_layer in enumerate(distances_for_hist.keys()):
        for measure_idx, measure_layer in enumerate(distances_for_hist.keys()):
            if measure_idx == 0:
                metamer_weights = np.ones(
                    len(distances_for_hist[metamer_layer][measure_layer])
                ) / len(distances_for_hist[metamer_layer][measure_layer])
                null_weights = np.ones(len(null[measure_layer])) / len(
                    null[measure_layer]
                )
            ax = plt.subplot(
                plot_size, plot_size, metamer_idx * plot_size + measure_idx + 1
            )
            plt.hist(
                distances_for_hist[metamer_layer][measure_layer],
                label="Metamers",
                color="b",
                weights=metamer_weights,
                range=distance_min_value_by_layer[measure_layer],
                **kwargs_hist,
            )
            plt.hist(
                null[measure_layer],
            ...
            if measure_idx == 0:
                if remove_fake_relu:
                    plt.ylabel(
                        "Metamer Layer: \n %s \n \n Frequency"
                        % (metamer_layer.split("_fake_relu")[0]),
                        fontsize=12,
                    )
                else:
                    plt.ylabel(
                        "Metamer Layer: \n %s \n \n Frequency" % metamer_layer,
                        fontsize=12,
                    )
...
def print_all_distance_measures_and_number_of_metamers(
    check_model_name,
    ...
    save_metamer_distances_location = os.path.join(
        check_model["location"],
    ...
    if use_saved_distances and os.path.isfile(save_metamer_distances_location):
        ...
        if network_type == "image":
            all_metamers_info = get_metamer_distances_imagenet(
                check_model["location"],
                check_model["metamer_folder"],
                check_model["layers"],
                metamer_subset,
            ...
            all_metamers_info = get_metamer_distances_wsn_word(
                check_model["location"],
                check_model["metamer_folder"],
                check_model["layers"],
                metamer_subset,
    ...
    all_null_distances = load_null_distributions(
        check_model["location"],
    ...
    if check_distance_measures is None:
        ...
        all_distance_measures_metamer_and_null = [
            ("spearman_rho", "spearman_rho", "max"),
    ...
    for distance_pair in all_distance_measures_metamer_and_null:
        ...
        if network_type == "audio":
            distance_list = print_number_of_metamers_passing_criteria_wsn_word(
                all_layer_correct,
                return_metamer_distances=True,
                distance_measure_each_metamer=distance_measure_each_metamer,
            )
        elif network_type == "image":
            distance_list = print_number_of_metamers_passing_criteria_imagenet(
                all_layer_correct,
                all_layer_correct_16_class,
                return_metamer_distances=True,
                distance_measure_each_metamer=distance_measure_each_metamer,
        ...
        overlap_idxs = print_null_dist(
            distance_measure_null,
            distance_list,
        ...
        if move_null_overlap_images:
            print(
                (
                    "MOVING THE IMAGES THAT OVERLAPED WITH THE NULL DISTIRBUTION INTO "
                ...
                for overlap_idx_list in overlap_idxs[layer]:
                    for overlap_idx in overlap_idx_list:
                        ...
                        overlap_dir = os.path.join(
                            os.path.dirname(overlap_file), "null_overlap_images"
                        )
                        move_file = os.path.join(
                            overlap_dir, os.path.basename(overlap_file)
                        )
                        try:
        ...
        if save_metamer_distances and not loaded_metamers_file:
            distance_hist_plot_path = os.path.join(
                plot_path,
        ...
        plot_metamer_vs_null(
            distance_measure_null,
            distance_measure_each_metamer,
            check_model["layers"],
