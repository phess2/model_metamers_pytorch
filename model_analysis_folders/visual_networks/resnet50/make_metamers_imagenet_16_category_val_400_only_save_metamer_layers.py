...
def run_audio_metamer_generation(
    SIDX,
    LOSS_FUNCTION,
    INPUTIMAGEFUNCNAME,
    RANDOMSEED,
    ...
    if MODEL_DIRECTORY is None:
        ...
        build_network_spec = importlib.util.spec_from_file_location(
            "build_network", os.path.join(MODEL_DIRECTORY, "build_network.py")
        )
    ...
    if use_dataset_preproc:  # Apply for some models, for instance if we have greyscale images or different sizes.
        image_dict["image"] = ds.transform_test(
            Image.fromarray(np.rollaxis(np.uint8(image_dict["image"] * 255), 0, 3))
        )
    ...
    label_idx = list(label_keys)[
        list(label_values).index(wnid_imagenet_name[image_dict["imagenet_category"]])
    ]
    ...
    base_filepath = os.path.join(
        MODEL_DIRECTORY, "metamers/%s/%d_SOUND_%s/" % (synth_name, SIDX, label_name)
    )
    ...
    try:
        ...
        print(f"Warning: Permission denied creating directory {base_filepath}: {e}")
        pass
    except OSError as e:
        print(f"Warning: Failed to create directory {base_filepath}: {e}")
        pass

    if os.path.isfile(pckl_path) and not overwrite_pckl:
        raise FileExistsError(
            "The file %s already exists, and you are not forcing overwriting"
            % pckl_path
        )
    ...
    for b in range(BATCH_SIZE):
        try:
            ...
            orig_predictions.append(
                predictions["signal/word_int"][b].detach().cpu().numpy()
            )

    # Get the predicted 16 category label
    orig_16_cat_prediction = [
        force_16_choice(np.flip(np.argsort(p.ravel(), 0)), CLASS_DICT["ImageNet"])
        for p in orig_predictions
    ]
    print("Orig Image 16 Category Prediction: %s" % (orig_16_cat_prediction))
    ...
    for layer_to_invert in metamer_layers:
        ...
        if hasattr(synth_kwargs["custom_loss"], "enable_dropout_flag"):
            ...
            model.enable_dropout_functions = synth_kwargs[
                "custom_loss"
            ]._enable_dropout_functions
            model.disable_dropout_functions = synth_kwargs[
                "custom_loss"
            ]._disable_dropout_functions
        ...
        this_loss, _ = calc_loss(
            model, im_n, invert_rep.clone(), synth_kwargs["custom_loss"]
        )
        ...
        print(
            "Step %d | Layer %s | Loss %f"
            % (synth_kwargs["iterations"], layer_to_invert, this_loss)
        )
        for i in range(NUMREPITER - 1):
            ...
            print(
                "Step %d | Layer %s | Loss %f"
                % (synth_kwargs["iterations"] * (i + 2), layer_to_invert, this_loss)
            )
        ...
        for key in all_outputs_out:
            if isinstance(all_outputs_out[key], dict):
        ...
        # Get the predicted 16 category label
        synth_16_cat_prediction = [
            force_16_choice(np.flip(np.argsort(p.ravel(), 0)), CLASS_DICT["ImageNet"])
            for p in synth_predictions
        ]
        print(
            "Layer %s, Synth Image 16 Category Prediction: %s"
    ...
    pckl_output_dict["predicted_16_cat_labels_out_dict"] = (
        predicted_16_cat_labels_out_dict
    )
    ...
    for key in all_outputs:
        if isinstance(all_outputs[key], dict):
    ...
    for layer_to_invert in metamer_layers:
        ...
        for (
            layer_to_measure
        ) in metamer_layers:  # pckl_output_dict['all_outputs_orig'].keys():
            met_rep = (
                pckl_output_dict["all_outputs_out_dict"][layer_to_invert][
                    layer_to_measure
                ]
                .numpy()
                .copy()
            ...
            orig_rep = (
                pckl_output_dict["all_outputs_orig"][layer_to_measure]
                .numpy()
                .copy()
                .ravel()
            )
    ...
    for layer_idx, layer_to_invert in enumerate(metamer_layers):
        ...
        for i in range(BATCH_SIZE):
            ...
            try:
                ...
                orig_predictions = (
                    predictions["signal/word_int"][i].detach().cpu().numpy()
                )
                synth_predictions = (
                    predictions["signal/word_int"][i].detach().cpu().numpy()
                )
            ...
            # Get the predicted 16 category label
            orig_16_cat_prediction = force_16_choice(
                np.flip(np.argsort(orig_predictions.ravel(), 0)), CLASS_DICT["ImageNet"]
            )
            synth_16_cat_prediction = force_16_choice(
                np.flip(np.argsort(synth_predictions.ravel(), 0)),
                CLASS_DICT["ImageNet"],
            )
            print(
                "Layer %s, Image %d, Orig Image 16 Category Prediction: %s"
                % (layer_to_invert, i, orig_16_cat_prediction)
            )
            print(
                "Layer %s, Image %d, Synth Image 16 Category Prediction: %s"
            ...
            if im[i].shape[0] == 3:
                plt.imshow(
                    (np.rollaxis(np.array(im[i].cpu().numpy()), 0, 3)),
                    interpolation="none",
                )
            elif im[i].shape[0] == 1:
                plt.imshow(
                    (np.array(im[i].cpu().numpy())[0, :, :]),
                    interpolation="none",
                    cmap="gray",
                )
            else:
                raise ValueError(
                    "Image dimensions are not appropriate for saving. Check dimensions"
                )
            plt.title(
                'Layer %s, Image %d, Predicted Orig Label "%s" \n Orig Coch'
                % (
                    layer_to_invert,
                    i,  # CLASS_DICT['ImageNet'][int(targ[i].cpu().numpy())]))
            ...
            if im[i].shape[0] == 3:
                plt.imshow(
                    (np.rollaxis(np.array(xadv[i].cpu().numpy()), 0, 3)),
                    interpolation="none",
                )
            elif im[i].shape[0] == 1:
                plt.imshow(
                    (np.array(xadv[i].cpu().numpy())[0, :, :]),
                    interpolation="none",
                    cmap="gray",
                )
            else:
            ...
            plt.title(
                'Layer %s, Image %d, Predicted Synth Label "%s" \n Synth Coch'
                % (
                    layer_to_invert,
                    i,  # CLASS_DICT['ImageNet'][int(targ[i].cpu().numpy())]))
            ...
            plt.scatter(
                np.ravel(np.array(all_outputs[layer_to_invert].cpu())[i, :]),
                np.ravel(all_outputs_out[layer_to_invert].cpu().detach().numpy()[i, :]),
            )
            plt.title(
                'Layer %s, Image %d, Label "%s" \n Optimization'
                % (
                    layer_to_invert,
                    i,
            ...
            if isinstance(all_outputs["final"], dict):
                dict_keys = list(
                    all_outputs["final"].keys()
                )  # So we ensure the same order
                plot_outputs_final = np.concatenate(
                    [
                        np.array(all_outputs["final"][task_key].cpu()[i, :]).ravel()
                ...
                plot_outputs_out_final = np.concatenate(
                    [
                        np.array(
                            all_outputs_out["final"][task_key]
                            .cpu()
                            .detach()
                            .numpy()[i, :]
                        ).ravel()
                ...
                plt.scatter(
                    np.ravel(np.array(all_outputs["final"].cpu())[i, :]),
            ...
            try:
                print(
                    'Layer %s, Image %d, Label "%s", Prediction Orig "%s", Prediction Synth "%s"'
                    % (
                        layer_to_invert,
                        i,
                        CLASS_DICT["ImageNet"][int(targ[i].cpu().numpy())],
                        CLASS_DICT["ImageNet"][
                            int(np.argmax(predictions[i].detach().cpu().numpy()))
                        ],
                        CLASS_DICT["ImageNet"][
                            int(np.argmax(predictions_out[i].detach().cpu().numpy()))
                        ],
                    )
                )
            except KeyError:
                print(
                    ...
                    % (
                        ...
                        CLASS_DICT["ImageNet"][
                            int(
                                np.argmax(
                                    predictions_out["signal/word_int"][i]
                                    .detach()
                                    .cpu()
                                    .numpy()
                                )
...
def main(raw_args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Input the sound indices and the layers to match"
    )
    parser.add_argument(
        "SIDX",
        metavar="I",
        type=int,
        help="index into the sound list for the time_average sound",
    )
    parser.add_argument(
        "-L",
        "--LOSSFUNCTION",
    ...
    parser.add_argument(
        ...
        metavar="--A",
        type=str,
        default="400_16_class_imagenet_val",
    ...
    parser.add_argument(
        "-DP", "--DATASET_PREPROC", action="store_true", dest="use_dataset_preproc"
    )
    parser.add_argument(
        "-E",
        "--NOISE_SCALE",
    ...
    parser.add_argument(
        ...
        default="sgd",
        choices=["sgd", "muon"],
        help="Optimizer to use for metamer generation (sgd or muon)",
    )
