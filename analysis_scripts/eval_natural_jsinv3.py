...
def run_word_in_noise_natural_eval(
    RANDOMSEED, MODEL_DIRECTORY, BATCH_SIZE, NUM_WORKERS
):
    ...
    if ds.__dict__.get("multitask_parameters", None) is not None:
        ...
        eval_kwargs["custom_adv_loss"] = partial(
            ds.multitask_parameters["calc_custom_adv_loss_with_batch_size"],
    ...
    print(store_out)
    args_dict = (
        eval_args.as_dict() if isinstance(eval_args, Parameters) else vars(eval_args)
    )
...
def main(raw_args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Input the arguments for word task evaluation"
    )
    parser.add_argument(
        "-R",
        "--RANDOMSEED",
