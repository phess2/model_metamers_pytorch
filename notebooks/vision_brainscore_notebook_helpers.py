...
def plot_brainscore_analysis_scatter(
    brainscore_df,
    models_to_evaluate,
    model_cmap_dict,
    model_style_dict,
    ...
    for bs_type in brain_comparisons:
        ...
        for m_idx, model_name in enumerate(models_to_evaluate):
            ...
            if len(brainscore_values) == 0:
                ...
                continue
            brainscore_extracted[model_name] = float(
                brainscore_values[bs_type].values[0]
            )

            brainscore_layer = brainscore_values[bs_type + " best layer"].values[0]

            try:
                average_metamer_recognition, sem_metamer_recognition = (
                    choose_metamer_layer(
            ...
            markers, caps, bars = plt.errorbar(
                brainscore_extracted[model_name],
        ...
        plt.text(
            0.015,
            0.25,
            "rho=%0.2f, p=%0.2f, (raw p=%0.2f)" % (rho_val, p_val_rho, p_val_rho_raw),
        )
        plt.text(
            0.015,
            0.15,
            "R^2=%0.2f, p=%0.2f, (raw p=%0.2f)" % (r_val**2, p_val_r, p_val_r_raw),
        )
