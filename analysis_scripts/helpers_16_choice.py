...
# Get the WNID
with open(
    os.path.join(FILE_DIRECTORY, "wordnetID_to_human_identifier.txt"), mode="r"
) as infile:
...
def force_16_choice(
    sorted_logit_args, class_labels_key, check_idx=0, class_index_offset=0
):
    """Finds the first label that falls into one of the 16
    categories, and assigns that label to the image"""
    check_predicted_label = class_labels_key[
        sorted_logit_args[check_idx] + class_index_offset
    ]
    try:
        predicted_label_in_16 = mapped_categories[
            wnid_imagenet_name[check_predicted_label]
        ]
        return predicted_label_in_16
    except KeyError:
        return force_16_choice(
            sorted_logit_args, class_labels_key, check_idx=check_idx + 1
        )
