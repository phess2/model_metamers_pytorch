...
class HannPooling2d(nn.Module):
    ...
    def forward(self, x):  # TODO: implement different padding
        ...
        if len(x_shape) == 2:  # Assume no batch or channel dimension
            ...
            b, c, h, w = x_shape
            x = x.view(b * c, 1, h, w)
        x = ch.nn.functional.conv2d(
            x, self.hann_window2d, stride=self.stride, padding=self.padding
        )
    ...
    def _make_hann_window(self):
        ...
        hann_window2d = np.expand_dims(
            np.expand_dims(np.outer(hann_window_h, hann_window_w), 0), 0
        )
