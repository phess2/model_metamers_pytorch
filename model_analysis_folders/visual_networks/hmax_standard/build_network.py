...
def build_net(ds_kwargs={}, return_metamer_layers=False):
    ...
    transform_hmax_train = transforms.Compose(
        [
            ...
            transforms.ToTensor(),
            da.Lighting(0.05, da.IMAGENET_PCA["eigval"], da.IMAGENET_PCA["eigvec"]),
    ...
    transform_hmax_test = transforms.Compose(
        [
            transforms.Resize(
                250
            ),  # Using the transforms for metamer generation and do not want to crop image.
    ...
    ds.scale_image_save_PIL_factor = (
        1  # Do not scale the output images by 255 when saving with PIL
    )
    ...
    model, _ = make_and_restore_model(
        arch="hmax_standard_with_readout",
        dataset=ds,
...
def main(return_metamer_layers=False, ds_kwargs={}):
    if return_metamer_layers:
