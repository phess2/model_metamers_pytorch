...
from .audio_functions.jsinV3DataLoader_precombined import (
    jsinV3_precombined,
...
class DataSet(object):
    ...
    def __init__(self, ds_name, data_path, **kwargs):
        ...
        assert set(required_args).issubset(set(kwargs.keys())), (
...
class ImageNet(DataSet):
    ...
    def get_model(self, arch, pretrained, arch_kwargs={}):
        ...
        return imagenet_models.__dict__[arch](
            num_classes=self.num_classes, pretrained=pretrained, **arch_kwargs
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        with open(
            os.path.join(
                robustness_path,
                "audio_functions/word_and_speaker_encodings_jsinv3.pckl",
            ),
            "rb",
        ) as f:
        ...
        if include_rep_in_model:
            ...
            if kwargs.get("audio_rep_on_gpu", False):
                ds_kwargs["audio_rep_transform"] = at.AudioToAudioRepresentation(
                    **AUDIO_REP_PROPERTIES
                )
    ...
    def get_model(self, arch, pretrained=False, arch_kwargs={}):
        ...
        if self.include_rep_in_model and (not self.use_normalization_for_audio_rep):
            return audio_models.custom_modules.SequentialAttacker(
                audio_models.custom_modules.AudioInputRepresentation(
                    **self.AUDIO_REP_PROPERTIES
                ),
        ...
        # for making adversarial coch examples for model trained with the sequential attacker above.
            ...
            return audio_models.__dict__[arch](
                num_classes=self.num_classes, **arch_kwargs
            )
