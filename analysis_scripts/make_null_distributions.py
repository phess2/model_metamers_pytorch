            ...
            activations_1.append(
                np.concatenate(
                    [
                        ...
                        .ravel()
                        .astype(np.float32)
                        for out_value in all_outputs[layer]
                    ],
                    0,
                )
            )
...
def run_null_distribution(
    NUMNULL, SPLITIDX, PATHNULL, RANDOMSEED, OVERWRITE_PICKLE, shuffle, MODEL_DIRECTORY
):
    BATCH_SIZE = 2
    NUM_WORKERS = 2
    ...
    pckl_path = os.path.join(
        MODEL_DIRECTORY,
        PATHNULL,
        "null_dist_N_%d_RS_%d_SPLIT_%d_START_%d.pckl"
        % (NUMNULL, RANDOMSEED, SPLITIDX, STARTIDX),
    )
    ...
    for null_iter in range(NUMNULL):
        ...
        with torch.no_grad():
            (predictions, rep, all_outputs), orig_image = model(
        ...
        for layer_idx, layer in enumerate(metamer_layers):
            if (np.sum(activations_1[layer_idx].ravel()) == 0) or (
                np.sum(activations_2[layer_idx].ravel()) == 0
            ):
                print(
                    "FOUND ALL ZEROS FOR LAYER %s, null_iter %d, SPLIT %d, SEED %d"
                    % (layer, null_iter, SPLITIDX, RANDOMSEED)
                )
                ...
                spearman_r_pair.append(
                    compute_spearman_rho_pair(
                        [activations_1[layer_idx], activations_2[layer_idx]]
                    )
                )
                pearson_r_pair.append(
                    compute_pearson_r_pair(
                        [activations_1[layer_idx], activations_2[layer_idx]]
                    )
                )
                db_SNR_i, norm_signal_i, norm_noise_i = compute_snr_db(
...
def main(raw_args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Input the arguments for the null distribution generation"
    )
    parser.add_argument(
        "-N",
        "--NUMNULL",
