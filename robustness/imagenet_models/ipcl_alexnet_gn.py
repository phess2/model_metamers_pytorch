...
class alexnet_gn(nn.Module):
    def __init__(
        ...
        self.fc8 = SequentialWithArgs(nn.Linear(4096, out_dim))
        if self._l2norm:
    ...
    def compute_feat(self, x, layer):
        if layer <= 0:
            return x
        x = self.conv_block_1(x)
        x = self.maxpool_block_1(
            x
        )  # moved max pooling out of this block for metamers project
        if layer == 1:
            return x
        x = self.conv_block_2(x)
        x = self.maxpool_block_2(
            x
        )  # moved max pooling out of this block for metamers project
        if layer == 2:
            return x
...
def ipcl_alexnet_gn(
    pretrained=False, progress=True, layer_for_fc=None, config=None, **kwargs
):
    """
    ...
    if config is None:
        raise ValueError(
            "This architecture requires a config file. See ipcl repository."
        )

    model = alexnet_gn(out_dim=config["out_dim"], l2norm=config["l2norm"], **kwargs)
    model.config = config
