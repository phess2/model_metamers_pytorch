...
class VisionNetworkTests(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        ...
        self.model_directory_base = os.path.join(
            self.base_directory, "model_analysis_folders", "visual_networks"
        )
        ...
        for model in VISION_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                ...
                make_metamers_imagenet_16_category_val_400_only_save_metamer_layers.main(
                    (
                        "0 -I 2 -N 2 -D model_analysis_folders/visual_networks/%s -O"
                        % model
                    ).split()
                )
        ...
        for model in VISION_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                ...
                make_null_distributions.main(
                    (
                        "-N 20 -I 0 -R 0 --shuffle -D model_analysis_folders/visual_networks/%s -O True"
                        % model
                    ).split()
                )
    ...
    def test_eval(self):
        for model in VISION_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                eval_natural_imagenet.main(
                    ("-D model_analysis_folders/visual_networks/%s" % model).split()
                )
...
class AudioNetworkTests(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        ...
        self.base_directory = os.path.dirname(os.path.abspath(__file__))
        self.model_directory_base = os.path.join(
            self.base_directory, "model_analysis_folders", "audio_networks"
        )
        ...
        for model in AUDIO_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                ...
                make_metamers_wsj400_behavior_only_save_metamer_layers.main(
                    (
                        "0 -I 2 -N 2 -D model_analysis_folders/audio_networks/%s -O"
                        % model
                    ).split()
                )
                pass
    ...
    def test_make_null(self):
        for model in AUDIO_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                ...
                make_null_distributions.main(
                    (
                        "-N 20 -I 0 -R 0 --no-shuffle -D model_analysis_folders/audio_networks/%s -O True"
                        % model
                    ).split()
                )
    ...
    def test_eval(self):
        for model in AUDIO_NETWORK_LIST[-1:]:
            with self.subTest(model=model):
                eval_natural_jsinv3.main(
                    ("-D model_analysis_folders/audio_networks/%s" % model).split()
                )
